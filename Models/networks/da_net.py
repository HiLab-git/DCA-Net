...
from Models.layers.channel_attention_layer import Domain_SE_Conv_Block


class Domain_Atten_Unet(nn.Module):
    def __init__(self, net_param):
        super(Domain_Atten_Unet, self).__init__()
        ...
        self.up4 = Domain_SE_Conv_Block(filters[4], filters[3], 1, self.num_adaptor, drop_out=True)
        self.up_concat3 = UpCat(filters[3], filters[2], self.is_deconv)
        self.up3 = Domain_SE_Conv_Block(filters[3], filters[2], 1, self.num_adaptor)
        self.up_concat2 = UpCat(filters[2], filters[1], self.is_deconv)
        self.up2 = Domain_SE_Conv_Block(filters[2], filters[1], 1, self.num_adaptor)
        self.up_concat1 = UpCat(filters[1], filters[0], self.is_deconv)
        self.up1 = Domain_SE_Conv_Block(filters[1], filters[0], 1, self.num_adaptor)
        ...
        self.final = nn.Sequential(nn.Conv2d(16, self.num_classes, kernel_size=1), nn.Softmax2d())
