...
from Models.networks.MetaEmbedding import DomadaptorEmbedding
...
class Decoder(nn.Module):
    def __init__(self, num_classes, num_domain, backbone, BatchNorm):
        ...
        self.relu = nn.ReLU()
        self.embedding = DomadaptorEmbedding(304, num_domain)
    ...
    def forward(self, x, feature, low_level_feat):
        ...
        dom_list = []
        x, hal_scale, dom_fea = self.embedding(feature)
        dom_list.append(dom_fea)

        x = self.last_conv(x)

        return x, feature , hal_scale, dom_list
