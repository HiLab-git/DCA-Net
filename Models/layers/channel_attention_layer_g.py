...
class Gen_Domain_Atten_Block(nn.Module):
    ...
    def forward(self, x, training=True):
        ...
        out = self.conv2(out)
        out = self.bn2(out)
        out, dof = self.domain_adaptor(out)
...
class Gen_Adapt_SE_block(nn.Module):
    ...
    def forward(self, x):
        ...
        recal_feature_att = recal_feature_att.reshape(recal_feature_att.size(0), self.num_block,
                                                      recal_feature_att.size(1)//self.num_block, -1)
        recal_feature_att = recal_feature_att[:,:,:,0]
        # for domain specific feature
        # domain_specific_feature = x.unsqueeze(dim=1).repeat(1, self.num_block, 1, 1, 1) * \
        #                           recal_feature_att.unsqueeze(dim=3).unsqueeze(dim=4)
        # domain_specific_mean = torch.mean(recal_feature_att, dim=2)
        # domain_specific_std = torch.std(recal_feature_att, dim=2)
        # domain_specific_sta = torch.stack((domain_specific_mean, domain_specific_std), dim=2)

        # for domain attention
        # for average pool
        domain_x_avg = self.glb_avg(domain_x)
        domain_x_avg = domain_x_avg.view(domain_x.size(0), -1)
        print(domain_x_avg.shape)
        domain_att_avg = self.fc1(domain_x_avg)
        domain_att_avg = self.relu(domain_att_avg)
        domain_att_avg = self.fc2(domain_att_avg)
        domain_att_avg = self.softmax(domain_att_avg)
        ...
        # d1, d2, d3 = domain_choose_list[0], domain_choose_list[1], domain_choose_list[2]
        domain_feature = [x*recal_feature_att[:, :, d:d+1].unsqueeze(dim=3) for d in domain_choose_list]
        # domain_feature_arr = [x*recal_feature_att[:, :, d:d+1].unsqueeze(dim=3) for d in domain_choose_list]
        # domain_specific_feature = torch.stack(domain_feature_arr, dim=1)
        # domain_specific_mean = torch.mean(domain_specific_feature, dim=(2, 3, 4))
        # domain_specific_std = torch.std(domain_specific_feature, dim=(2, 3, 4))
        # domain_specific_sta = torch.stack((domain_specific_mean, domain_specific_std), dim=2)
        # d1_recal_att = recal_feature_att[:, :, d1:d1+1].unsqueeze(dim=3)
        # d2_recal_att = recal_feature_att[:, :, d2:d2+1].unsqueeze(dim=3)
        # d3_recal_att = recal_feature_att[:, :, d3:d3+1].unsqueeze(dim=3)
        # d1_recal_att = recal_feature_att[:, :, d1:d1+1]
        # d2_recal_att = recal_feature_att[:, :, d2:d2+1]
        ...
        domain_recal_feature = x * domain_recal_att + residual
        # domain1_feature = x * d1_recal_att
        # domain2_feature = x * d2_recal_att
        # domain3_feature = x * d3_recal_att
        # print(domain_recal_feature.shape)

        # return domain_recal_feature, domain1_feature, domain2_feature
        return domain_recal_feature, domain_feature
