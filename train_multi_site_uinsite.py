...
from Datasets.Mul_prostate.joint_Multi_Dataset import joint_Multi_Dataset
...
def train(model: torch.nn.Module, loader_train: list, loader_val: DataLoader, train_params: dict, writer):
    ...
    for iter_num in tqdm(range(train_params['start_iter'], train_params['iterations']),
        ...
        for i in range(len(domain_feature)):
            ...
            loss_domain_sdf_all += 1 - math.sqrt(loss_domain_sdf / len(domain_feature[i]))
        ...
        # loss = 0.5*(loss_bce+loss_dice) + compact_loss
        # 反向传播更新参数
        optimizer.zero_grad()
        loss.backward()
