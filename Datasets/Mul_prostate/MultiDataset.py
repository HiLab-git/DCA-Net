...
class MultiDataset(Dataset):
    def __init__(self, npz_dir='./data/npz_data', datalist_dir='./Datasets/Mul_s', ignore_site=None, mode='train'):
        '''
        ...
        if self.mode == 'train':
            ...
            with open(os.path.join(self.datalist_dir, self.mode + '_data_list.list'),
                      'r') as f:
                self.image_list = f.readlines()
                self.image_list = [item.replace('\n', '') for item in self.image_list if ignore_site in item]
                self.image = [os.path.join(self.npz_dir, x + '.npz') for x in self.image_list]
        logging.info(f'Creating {self.mode} dataset in {self.npz_dir} with {len(self.image)} slices in total.')
...
if __name__ == '__main__':
    ...
    dataset = MultiDataset(npz_dir='./data/npz_data', datalist_dir='./Datasets/Mul_s', mode='train')
