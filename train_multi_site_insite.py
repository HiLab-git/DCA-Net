...
from Datasets.joint_Multi_Dataset import joint_Multi_Dataset
from Datasets.Meta_Multi_Dataset import RandomCrop, RandomNoise, RandomRotFlip, ToTensor
from utils.dice_loss import get_soft_label, SoftDiceLoss, val_dice, val_dice_class, connectivity_region_analysis
from utils.dice_loss import get_compactness_cost
...
def train(model: torch.nn.Module, loader_train: list, loader_val: DataLoader, train_params: dict, writer):
    ...
    for iter_num in tqdm(range(train_params['start_iter'], train_params['iterations']),
        ...
        for i in range(len(domain_feature)):
            ...
            loss_domain_sdf_all += 10**-i * (1 - math.sqrt(loss_domain_sdf / len(domain_feature[i])))
...
if __name__ == '__main__':
    ...
    train_dataset = [joint_Multi_Dataset(npz_dir=data_params['npz_data_dir'], datalist_list=site_list, mode='train',
                                         unseen_site=data_params['unseen_site'][:-1],
                                         transform=transforms.Compose([
                                            # RandomRotFlip(),
                                            # RandomNoise(),
                                            ToTensor(),
                                        ])) for site_list in train_file_list]
    valid_dataset = joint_Multi_Dataset(npz_dir=data_params['npz_data_dir'], datalist_list=valid_file_list[0], mode='val',
                                        unseen_site=data_params['unseen_site'][:-1],
                                        transform=transforms.Compose([
                                           ToTensor(),
                                        ]))
